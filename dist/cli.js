...
program
    ...
    .option('-m, --module <modules>', 'Comma-separated list of modules to enable', 'core,skills,commands')
...
program
    ...
    try {
        ...
        for (const r of results) {
            logger.info(`${r.moduleName}: ${r.copied.length} copied, ${r.skipped.length} skipped, ${r.errors.length} errors`);
        }
    }
    catch (err) {
        logger.error(err instanceof Error ? err.message : String(err));
...
program
    ...
    try {
        ...
        logger.divider();
        for (const r of results) {
            logger.info(`${r.moduleName}: ${r.copied.length} restored, ${r.skipped.length} skipped, ${r.errors.length} errors`);
        }
    }
    catch (err) {
        logger.error(err instanceof Error ? err.message : String(err));
